...
display(Image('reconstruction_gpu.png'))
display(Image('reconstruction_gpu_detailed.png'))
...
==================================================
RECONSTRUCTION STATISTICS - GPU Autoencoder
==================================================
...
Plot saved to: reconstruction_gpu.png
Detailed plot saved to: reconstruction_gpu_detailed.png
```
...
```bash
# 1. Train GPU model
./autoencoder_gpu ./cifar-10-batches-bin 5 32 500 adam

# File được tạo tự động:
# - training_history_gpu.csv (training history)
# - reconstructed_images_gpu.bin (reconstruction samples)
# - autoencoder_gpu_final.bin (model weights)
# - autoencoder_gpu_best.bin (best model weights)
...
# Copy cả 2 file reconstruction .bin về cùng thư mục rồi chạy:
python visualize_reconstruction.py --compare
```
